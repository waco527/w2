...
class OptimizerUtils
{
    ...
    public static function replace_bg($css)
    {
        ...
        if (TWO_LAZYLOAD && isset($two_bg_lazyload) && ($two_bg_lazyload == "on" || ($two_bg_in_viewport_lazyload == "on" && $critical->bg_images_in_viewport)) && !$critical->use_uncritical) {
            // phpcs:ignore Squiz.PHP.CommentedOutCode.Found
            ...
            if (isset($matches) && is_array($matches) && isset($matches[1])) {
                ...
                foreach ($images as $src) {
                    ...
                    if($flag_continue === false && $two_bg_in_viewport_lazyload == "on" && $critical->bg_images_in_viewport){
                      $abs_src = self::get_absolute_url($src);
                      $flag_continue = in_array($abs_src, $critical->bg_images_in_viewport);
                    }
    ...
    public static function inject_in_html($content, $payload, $where)
    {
        ...
        if (false !== $position) {
            ...
            if ('after' === $where[1]) {
                ...
                $tagEndPosition = self::strpos($afterTag, '>');
    ...
    public static function get_worker_script(){
        ...
        $two_exclude_uncritical_fonts = 0;
        if($TwoSettings->get_settings('two_exclude_uncritical_fonts') == 'on'){
            $two_exclude_uncritical_fonts = 1;
        }
        return '
        <script data-pagespeed-no-defer ' . OptimizerScripts::TWO_NO_DELAYED_JS_ATTRIBUTE . ' type="text/javascript">
         ...
         <script data-pagespeed-no-defer ' . OptimizerScripts::TWO_NO_DELAYED_JS_ATTRIBUTE . ' id="two_worker" type="javascript/worker">
            let two_exclude_uncritical_fonts = '.$two_exclude_uncritical_fonts.';
    ...
    public static function get_tenweb_connection_link( $endpoint = 'sign-up', $args = [] ){
      ...
      set_site_transient(TW_OPTIMIZE_PREFIX . '_saved_nonce', $token, 60 * 20 );
